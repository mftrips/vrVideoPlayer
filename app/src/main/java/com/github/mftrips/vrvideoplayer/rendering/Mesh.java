...
public final class Mesh {
     ...
     * @param verticalFovDegrees   Total latitudinal degrees that are covered by the sphere. Must
     *                             be in
     *                             (0, 180].
    ...
    public static Mesh createUvSphere(
        ...
        // Each latitude strip has 2 * (longitudes quads + extra edge) vertices + 2 degenerate
        // vertices.
        ...
        for (int j = 0; j < latitudes; ++j) { // For each horizontal triangle strip.
            // Each latitude band lies between the two phi values. Each vertical edge on a band lies
            // on a theta value.
    ...
    void glDraw(float[] mvpMatrix, int eyeType) {
        ...
        int textureOffset =
                (eyeType == Eye.Type.RIGHT) ? POSITION_COORDS_PER_VERTEX + 2 :
                        POSITION_COORDS_PER_VERTEX;
